        ...
        <ol className="pt-6 list-decimal list-inside space-y-2 text-left w-full">
          {bucketList.map((item) => (
            ...
            >
              {item.text}
              {item.link && (
                <>{" "}<a href={item.link} target="_blank" rel="noopener noreferrer" className="text-primary no-underline underline-offset-4 transition-all duration-200 ease-in-out hover:text-title">{item.linkText}</a></>
              )}
            </li>
          ))}
        </ol>
